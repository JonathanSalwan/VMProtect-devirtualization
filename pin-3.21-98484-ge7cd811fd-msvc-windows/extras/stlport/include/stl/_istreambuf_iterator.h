...
public:
  istreambuf_iterator(streambuf_type* __p = 0) : _M_buf(NULL), _M_c(_CharT()), _M_eof(__p == 0), _M_have_c(false) { this->_M_init(__p); }
...
inline istreambuf_iterator<_CharT, _Traits>::istreambuf_iterator(basic_istream<_CharT, _Traits>& __is) :
    _M_buf(NULL), _M_eof(_M_get_istreambuf(__is) == 0),_M_have_c(false), _M_c(_CharT())
