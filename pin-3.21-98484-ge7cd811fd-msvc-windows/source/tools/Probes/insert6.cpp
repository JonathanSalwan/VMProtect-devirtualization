...
VOID Before_Malloc(size_t size) { cout << "Before_Malloc: Calling my_malloc() with size=" << size << endl << flush; }
...
VOID Before_Free(void* ptr) { cout << "Before_Free: Calling my_free() with ptr=" << hex << (ADDRINT)ptr << dec << endl << flush; }
...
VOID ImageLoad(IMG img, VOID* v)
{
    ...
    if (RTN_Valid(rtn))
    {
        ...
        cout << "Inserting calls before/after my_malloc in " << IMG_Name(img) << endl;

        PROTO proto_malloc = PROTO_Allocate(PIN_PARG(void*), CALLINGSTD_DEFAULT, "my_malloc", PIN_PARG(size_t), PIN_PARG_END());

        RTN_InsertCallProbed(rtn, IPOINT_BEFORE, AFUNPTR(Before_Malloc), IARG_PROTOTYPE, proto_malloc,
    ...
    if (RTN_Valid(rtn))
    {
        ...
        cout << "Inserting calls before/after my_free in " << IMG_Name(img) << endl;

        PROTO proto_free = PROTO_Allocate(PIN_PARG(void), CALLINGSTD_DEFAULT, "my_free", PIN_PARG(void*), PIN_PARG_END());

        RTN_InsertCallProbed(rtn, IPOINT_BEFORE, AFUNPTR(Before_Free), IARG_PROTOTYPE, proto_free, IARG_FUNCARG_ENTRYPOINT_VALUE,
                             0, IARG_END);
