...
typedef void(__stdcall* StdCallFunctionToBeReplacedByPinType)(char c1, int num1, char c2, int num2);

// the calling convention of ReplacementFunc should be the default calling convention.
void /*__stdcall*/ ReplacementFunc(StdCallFunctionToBeReplacedByPinType originalFunc, char c1, int num1, char c2, int num2)
...
void DoReplacementFunc(IMG img, char* funcName)
{
    ...
    if (RTN_IsSafeForProbedReplacement(rtnToReplace))
    {
        ...
        PROTO protoOfStdCallFunction1ToBeReplacedByPin =
            PROTO_Allocate(PIN_PARG(void*), CALLINGSTD_STDCALL, "protoOfStdCallFunction1ToBeReplacedByPin", PIN_PARG(char),
        ...
        RTN_ReplaceSignatureProbed(rtnToReplace, AFUNPTR(ReplacementFunc), IARG_PROTOTYPE,
                                   protoOfStdCallFunction1ToBeReplacedByPin, IARG_ORIG_FUNCPTR, IARG_FUNCARG_ENTRYPOINT_VALUE, 0,
